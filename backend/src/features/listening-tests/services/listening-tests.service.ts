...
import { CreateListeningTestDto, PublishListeningTestDto, UpdateListeningTestDto } from '../dto/save-listening-test.dto';
import { ListListeningTestsQueryDto } from '../dto/list-listening-tests-query.dto';
import { ListeningTestsRepository, RepositoryMutation } from '../repositories/listening-tests.repository';
import { ListeningTestContentService } from './listening-test-content.service';
import { ListeningAudit, ListeningTestAggregate, ListeningTestSummary } from '../types/listening-test.type';

@Injectable()
export class ListeningTestsService {
  constructor(
    private readonly repository: ListeningTestsRepository,
    private readonly content: ListeningTestContentService,
  ) {}

  async list(query: ListListeningTestsQueryDto, actor: AuthUser): Promise<PaginatedResponse<ListeningTestSummary>> {
    const result = await this.repository.list(query, actor);
    return paginate(result.tests, result.total, query);
  }

  async listPublished(query: ListListeningTestsQueryDto): Promise<PaginatedResponse<ListeningTestSummary>> {
    const result = await this.repository.listPublished(query);
    return paginate(result.tests, result.total, query);
  }

  async get(id: string, actor: AuthUser): Promise<ListeningTestAggregate> {
  ...
  async publish(id: string, actor: AuthUser, dto: PublishListeningTestDto, audit: ListeningAudit): Promise<ListeningTestAggregate> {
    const aggregate = await this.repository.findAggregate(id);
    if (!aggregate || aggregate.status === 'ARCHIVED') this.notFound();
    await this.assertOwner(id, actor);
  ...
  async archive(id: string, actor: AuthUser, audit: ListeningAudit): Promise<void> {
  ...
  private async assertOwner(id: string, actor: AuthUser): Promise<void> {
    ...
    if (actor.role === 'TEACHER' && owner.createdBy !== actor.id) {
      throw new ApplicationError('LISTENING_TEST_FORBIDDEN', 'You can only manage tests that you created.', 403);
    }
  }

  private unwrap(result: RepositoryMutation): ListeningTestAggregate {
  ...
  private notFound(): never {
    throw new ApplicationError('LISTENING_TEST_NOT_FOUND', 'Listening test not found.', 404);
  }

  private versionConflict(): never {
    throw new ApplicationError(
      'LISTENING_TEST_VERSION_CONFLICT',
