...
export class TokenService {
  ...
  async refresh(rawToken: string, metadata: ClientMetadata): Promise<TokenPair> {
    ...
    const nextToken = await this.signRefresh(user.id, nextId, payload.family);
    ...
    return { accessToken: await this.signAccess(user), refreshToken: nextToken, expiresIn: this.accessTtl() };
  }
  ...
  private async createPair(user: AuthUser, familyId: string, metadata: ClientMetadata, persist: boolean): Promise<TokenPair> {
    ...
    const refreshToken = await this.signRefresh(user.id, tokenId, familyId);
    if (persist) {
    ...
    return { accessToken: await this.signAccess(user), refreshToken, expiresIn: this.accessTtl() };
  }
  ...
  private signRefresh(userId: string, tokenId: string, familyId: string): Promise<string> {
    const payload: RefreshTokenPayload = { sub: userId, jti: tokenId, family: familyId, type: 'refresh' };
    return this.jwt.signAsync(payload, {
  ...
  private async verifyRefresh(token: string): Promise<RefreshTokenPayload> {
    try {
      ...
      if (payload.type !== 'refresh' || !payload.jti || !payload.family) throw new Error('Invalid token type');
      return payload;
    } catch {
