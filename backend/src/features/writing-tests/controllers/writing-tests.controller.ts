...
@ApiTags('Writing Test Management')
@ApiCookieAuth('aptimate_access_token')
@Controller('admin/writing-tests')
@UseGuards(JwtAuthGuard, RolesGuard)
@Roles('ADMIN', 'TEACHER')
export class WritingTestsController {
  constructor(private readonly tests: WritingTestsService) {}

  @Get()
  list(@Query() query: ListWritingTestsQueryDto, @CurrentUser() actor: AuthUser) { return this.tests.list(query, actor); }

  @Get(':id')
  get(@Param('id', new ParseUUIDPipe()) id: string, @CurrentUser() actor: AuthUser) { return this.tests.get(id, actor); }

  @Post()
  create(@Body() dto: CreateWritingTestDto, @CurrentUser() actor: AuthUser, @Req() request: Request) {
    return this.tests.create(actor, dto, this.audit(request));
  }

  @Put(':id')
  update(@Param('id', new ParseUUIDPipe()) id: string, @Body() dto: UpdateWritingTestDto, @CurrentUser() actor: AuthUser, @Req() request: Request) {
  ...
  async archive(@Param('id', new ParseUUIDPipe()) id: string, @CurrentUser() actor: AuthUser, @Req() request: Request): Promise<void> {
    await this.tests.archive(id, actor, this.audit(request));
  }

  private audit(request: Request): WritingAudit {
    const requestId = request.headers['x-request-id'];
    return { requestId: typeof requestId === 'string' && UUID_PATTERN.test(requestId) ? requestId : undefined, ipAddress: request.ip };
  }
}
