...
import { BookHeart, BookOpen, Check, ChevronLeft, ChevronRight, Headphones, Heart, Lightbulb, Plus, Search, RotateCcw, Shuffle, Volume2, X } from 'lucide-react';
...
export default function DictationPage() {
  ...
  return (
    <div className={styles.page}>
      <nav className={styles.modeTabs} aria-label="Practice mode">
        <button className={mode === 'dictation' ? styles.activeMode : ''} onClick={() => setMode('dictation')}><Headphones size={19} /> Dictation</button>
        <button className={mode === 'flashcard' ? styles.activeMode : ''} onClick={() => setMode('flashcard')}><BookOpen size={19} /> Flashcard</button>
        <button className={mode === 'notebook' ? styles.activeMode : ''} onClick={() => setMode('notebook')}><BookHeart size={19} /> Notebook</button>
      </nav>
