...
export default function useAdminWritingTests({ enabled, search, mode, status, page, pageSize }) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      writingTestsApi.listAdmin({ search, mode, status, page, pageSize, signal: controller.signal })
        ...
        .catch(error => {
          if (error.code !== 'ERR_CANCELED') setState(current => ({ ...current, data: [], loading: false, error: getApiError(error, 'Unable to load Writing tests.') }));
        });
    }, search?.trim() ? 300 : 0);
