...
export default function WritingTestDetailsPage() {
  ...
  const uploadPicture = async (event) => {
    ...
    try { updateDetails('pictureUrl', await resizeImage(file, 1400, 900, { output: 'dataURL', mimeType: 'image/jpeg', quality: .82 })); setErrors((current) => ({ ...current, pictureUrl: undefined })); setMessage('Picture uploaded successfully.'); }
    catch { setErrors((current) => ({ ...current, pictureUrl: 'Unable to process this image.' })); }
  };

  const saveInformation = () => { const next = validateWritingDetails(test.details); setErrors(next); if (!hasValidationErrors(next)) setMessage('Test information saved in the current draft.'); };
  const saveTest = () => {
    const validation = validateWritingTest(test);
    setErrors(validation.details);
    if (hasWritingTestErrors(validation)) { const incomplete = Object.entries(validation.parts).filter(([, value]) => hasValidationErrors(value)).map(([part]) => `Part ${part}`).join(', '); setMessage(`Complete all required content${incomplete ? ` in ${incomplete}` : ''} before saving.`); return; }
    try { const saved = saveStoredWritingTest(test); navigate(`/admin/tests/writing/${saved.id}/preview`, { state: { toast: test.id ? 'Writing test updated successfully.' : 'Writing test created successfully.' } }); }
    catch { setMessage('The image is too large to save in this browser. Please upload a smaller image.'); }
  };

  return <div className={styles.page}><div className={styles.crumb}><b>Test Management</b><ChevronRight /><b>Admin</b><ChevronRight /><span>{test.details.title || 'New test'}</span></div><section className={styles.information}><h2>INFORMATION TEST</h2><div className={styles.infoGrid}><div className={styles.fields}><label><b>Title:</b><input value={test.details.title} onChange={(event) => updateDetails('title', event.target.value)} />{errors.title && <small>{errors.title}</small>}</label><label><b>Source:</b><input value={test.details.source} onChange={(event) => updateDetails('source', event.target.value)} />{errors.source && <small>{errors.source}</small>}</label><label><b>Picture:</b><button type="button" onClick={() => fileInputRef.current?.click()}><ImagePlus />Upload image</button><input ref={fileInputRef} className={styles.fileInput} type="file" accept="image/*" onChange={uploadPicture} />{errors.pictureUrl && <small>{errors.pictureUrl}</small>}</label><button className={styles.saveInfo} onClick={saveInformation}>Save</button>{message && <p className={styles.message}>{message}</p>}</div><aside><b>Preview</b><div><header><span>{test.mode === 'full' ? 'Full test' : test.mode.replace('part', 'Part ')}</span><small>Not Started</small></header>{test.details.pictureUrl ? <img src={test.details.pictureUrl} alt="Test preview" /> : <strong>AptiMate<br /><em>Writing</em></strong>}<button onClick={saveTest}>Preview</button></div></aside></div></section><section className={styles.content}><h2>CONTENT TEST</h2><div>{visibleParts.map((part) => <PartSummaryCard key={part.number} part={part} onEdit={() => navigate(`${basePath}/part/${part.number}`)} />)}</div><button className={styles.saveAll} onClick={saveTest}>{test.id ? 'Update test & preview' : 'Save test & preview'}</button></section></div>;
}
