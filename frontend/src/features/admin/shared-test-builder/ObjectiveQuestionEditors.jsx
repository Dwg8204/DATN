...
export function MultipleChoiceQuestionEditor({ question, index, onChange }) {
  ...
  return <article className={styles.questionCard}><header><strong>Question {index + 1}</strong><span>Single answer</span></header><label>Question<input value={question.text} onChange={(event) => onChange({ ...question, text: event.target.value })} placeholder="Enter the question..."/></label><div className={styles.options}>{question.options.map((option, optionIndex) => <label key={optionIndex} className={styles.option}><input type="radio" name={`correct-${index}`} checked={question.correctAnswer === optionIndex} onChange={() => onChange({ ...question, correctAnswer: optionIndex })}/><b>{String.fromCharCode(65 + optionIndex)}</b><input value={option} onChange={(event) => updateOption(optionIndex, event.target.value)} placeholder={`Option ${String.fromCharCode(65 + optionIndex)}`}/></label>)}</div></article>;
}
