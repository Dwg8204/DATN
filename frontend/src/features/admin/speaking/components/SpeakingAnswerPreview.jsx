...
const Questions=({questions,seconds})=><div className={styles.questions}>{questions.map((question,index)=><article key={question.id}><span>{index+1}</span><RichTextContent value={question.text}/><small>{seconds}</small></article>)}</div>;
